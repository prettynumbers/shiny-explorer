...
```{r cor}
with(mydf, cor(numeric1, numeric2, use = "pairwise.complete.obs", method = "pearson"))
```

Test of significance of correlation coefficient

```{r cortest}
with(mydf, cor.test(numeric1, numeric2, method = "pearson"))
```
...
```{r r2}
with(mydf, cor(numeric1, numeric2, use = "pairwise.complete.obs")^2)
```
