...
require(plyr)
kable(
  ddply(mydf, "factor1", summarize, 
        numeric1.mean = mean(numeric1), 
        numeric1.min = min(numeric1), 
        numeric1.max = max(numeric1)
  ), 
  format = "html", row.names = NA, table.attr = 'class="data table table-bordered table-condensed"'
)
```
